...
{	
  ...
  {
	    //set center target
	    MovementEngine me = GameState._weaponList.elementAt(_selected);
	    ...
	    for (int i = 0; i < GameState._weaponList.size(); i ++)
	    {
	    	if (i != _selected)
	    	{	
		        me = (MovementEngine)GameState._weaponList.elementAt(i);
  ...
  {
	  if (GameState._weaponList.size() > 1)
	  {
		  _selected = _selected + 1;
		  if (_selected > (GameState._weaponList.size() - 1))
		  {
			  _selected = 0;
		  }
	  }
	...
	public void keyReleased(KeyEvent arg0)
	{
		...
		{	
			//launch missile
			double a = GameUtils.getA(GameState._weaponList.get(0).getX(), _selectedTarget.getX());
			double b = GameUtils.getB(GameState._weaponList.get(0).getY(), _selectedTarget.getY());
			double track = GameUtils.track(a, b);
			MovementEngine missile = new FollowEngine((int)track, (int)track, (int)GameState._weaponList.get(0).getX()
					, (int)GameState._weaponList.get(0).getY(), 1, 10, 1, 1, Constants.MISSILE", _selectedTarget,  GameState._weaponList.get(0), 1000);
		...
		{	
			...
			double b = GameUtils.getB(GameState._weaponList.get(0).getY(), _selectedTarget.getY());
			double track = GameUtils.track(a, b);
			MovementEngine bullet = new LineEngine((int)track, (int)track, (int)GameState._weaponList.get(0).getX()
					, (int)GameState._weaponList.get(0).getY(),25, 25, 1, 1, "gun", GameState._weaponList.get(0), 500);
